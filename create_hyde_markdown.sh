...
blacklist=combinatorics,detail,utils,terms,mpi,indexing
...
for src in ${sources[@]}; do
    echo Compiling $src
    hyde -hyde-yaml-dir=$target_dir -hyde-update $src --namespace-blacklist=$blacklist -- -x c++ -std=c++17 -c -DLILA_USE_ACCELERATE -Wno-pragma-once-outside-header $includes >> $logfile 2>&1
